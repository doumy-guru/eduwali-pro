...
import { getFirestore, doc, setDoc, getDoc, onSnapshot, collection, query, addDoc, updateDoc, deleteDoc, enableNetwork, disableNetwork } from 'firebase/firestore';
import { Users, BookOpen, AlertCircle, CheckCircle, FileSpreadsheet, Plus, Search, ChevronRight, User, Key, LogOut, ArrowLeft, Send, Download, Settings, Trash2, Edit3, ShieldAlert } from 'lucide-react';
                ...
                {viewMode === 'login' ? (
                    <div className="space-y-6">
                        <div className="text-center mb-6">
                            <h2 className="text-xl font-bold text-gray-800 mb-2">Selamat Datang Pahlawan Tanpa Tanda Jasa</h2>
                            <p className="text-sm text-gray-500">Masuk untuk mengelola kelas dan siswa bimbingan Anda dengan lebih efisien.</p>
                        </div>
                        <button onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-200 text-gray-700 py-3.5 rounded-xl hover:bg-gray-50 hover:border-indigo-200 font-bold transition-all shadow-sm group">
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-72 bg-white border-r border-gray-200 flex flex-col shrink-0 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} shadow-2xl md:shadow-none`}>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50">
            <div className="flex items-center gap-3 px-4 py-3 bg-white rounded-xl mb-3 shadow-sm border border-gray-100">
                <img src={user?.photoURL || `https://ui-avatars.com/api/?name=${user?.email}&background=4f46e5&color=fff`} alt="Profile" className="w-10 h-10 rounded-full bg-indigo-100" />
                <div className="truncate flex-1">
                    <p className="font-bold text-sm text-gray-800 truncate">{user?.displayName || 'Guru'}</p>
